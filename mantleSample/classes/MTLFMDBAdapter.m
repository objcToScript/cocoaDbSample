...
- (id)initWithFMResultSet:(FMResultSet *)resultSet modelClass:(Class)modelClass error:(NSError **)error {
    ...
    */
    ...
    //dictionaryValueはモデルに書いたカラム名からResultSETに問い合わせて値を取得しmodelにつめる
...
- (NSValueTransformer *)FMDBTransformerForKey:(NSString *)key {
	...
	SEL selector = (SEL)MTLSelectorWithKeyPattern(key, "FMDBTransformer");
...
+ (NSMutableArray *)insertStatementFor_keyPath:(MTLModel<MTLFMDBSerializing> *)model {
    NSDictionary *columns = [model.class FMDBColumnsByPropertyKey];
    NSSet *propertyKeys = [model.class propertyKeys];
    NSArray *Keys = [[propertyKeys allObjects] sortedArrayUsingSelector:@selector(compare:)];
...
+ (NSMutableArray *)updateStatementFor_updateStatementFor_keyPath:(MTLModel<MTLFMDBSerializing> *)model {
    NSDictionary *columns = [model.class FMDBColumnsByPropertyKey];
    NSSet *propertyKeys = [model.class propertyKeys];
    NSArray *Keys = [[propertyKeys allObjects] sortedArrayUsingSelector:@selector(compare:)];
...
+ (NSString *)whereStatementForModel:(MTLModel<MTLFMDBSerializing> *)model
{
    ...
    for (NSString *key in keys) {        
        NSString* key2 = [self _isYoyakugo:key];
        NSString *s = [NSString stringWithFormat:@"%@ = ?", key2];
        [where addObject:s];
    }
